...
uses
  ...
  Vcl.ComCtrls,
  G2D,G2DCallDll,G2DTypes, Vcl.Imaging.jpeg, VclTee.TeeGDIPlus, VCLTee.TeEngine,
  VCLTee.Series, VCLTee.TeeProcs, VCLTee.Chart;

const
CHUNK_SIZE =8*1024;         //* Amount of bytes we are sending in each buffer */
...
start:=0;
//sync scope
while (Form1.Data[start]>0) and (start<500) do inc(start);
while (Form1.Data[start]<0) and (start<500) do inc(start);
Series1.clear;
Chart1.Axes.Left.SetMinMax(-1, 1); //set the chart y scale
//set data in chart (series)
for I := 0 to 1024 do
  Series1.Addxy(I*1000/SAMPLE_RATE,Form1.Data[start+I]/32000);
end;
//******************************************
function new_sample(sink :PGstElement; data:pointer):TGstFlowReturn; cdecl;
var
...
//* Retrieve the buffer */
  ...
  if Assigned(sample) then
    begin
    buffer := _Gst_sample_get_buffer(sample);
    if Assigned(buffer) then
      begin
      // Map the buffer for reading
      _Gst_buffer_map(buffer, @map, GST_MAP_READ);
      move(map.data^,Form1.Data,map.size);//Move data from buffer to the form data
      _Gst_buffer_unmap(buffer, @map);
      _Gst_buffer_unref(buffer)
      end;
