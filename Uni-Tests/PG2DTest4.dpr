...
uses
  ...
  WinConsoleFunction in '..\Examples\WinConsoleFunction.pas',
...
begin
  UriParameter:='https://www.freedesktop.org/'+
  ...
  try
    ...
    if not GStreamer.BuildAndPlay('playbin name=player uri=' + UriParameter) then     // name=player
    ...
    begin
      if once and (GStreamer.State = GST_STATE_PLAYING) then
        begin
        once:=false;
        Writeln('pipeline='+GStreamer.Pipeline.GetName);
        Elem := GStreamer.FindElement('player');
        if Elem<>nil
          then writeln( Elem.GetName )
          else writeln('fail');
